...
public final class SentryTask implements Runnable {
    ...
    public SentryTask(SentryService service, SentryRepository repository, SanctuaryRepository sanctuaryRepository, Logger logger) {
        this.service=service;this.repository=repository;this.sanctuaryRepository=sanctuaryRepository;this.logger=logger;
    }

    public void start(JavaPlugin plugin) { Bukkit.getScheduler().runTaskTimer(plugin, this, 10L, 10L); }
    ...
    @Override public void run() {
        try {
            ...
            scanTriggers();
        } catch (SQLException exception) { logger.log(Level.WARNING, "Failed sentry maintenance tick", exception); }
    }
    ...
    private void tickSentry(SentryRecord sentry, Instant now) throws SQLException {
        ...
        if (!TerritoryCalculator.contains(sanctuary.position().orElseThrow(), sanctuary.territoryRadius(), sentry.world(), loc.getX(), loc.getZ())) {
            service.markDown(sentry); return;
        }
        ...
        if (sentry.state() == SentryState.RECALLING) {
            ...
            if (distance <= SentryService.HOME_REACHED_DISTANCE) {
                service.setDisabled(sentry, false); return;
            }
            if (sentry.recallDeadline().isPresent() && !now.isBefore(sentry.recallDeadline().orElseThrow())) {
                entity.teleport(service.home(sentry).clone().add(0,1,0)); service.setDisabled(sentry, false); return;
            }
            service.moveHome(mob, sentry); return;
        }
        LivingEntity target = mob.getTarget();
        SentryDefinition definition = service.definition(sentry).orElse(null);
        if (target != null && definition != null) {
            Location home = service.home(sentry);
            double dx = target.getLocation().getX()-home.getX(), dz=target.getLocation().getZ()-home.getZ();
            boolean targetInSanctuary = TerritoryCalculator.contains(sanctuary.position().orElseThrow(), sanctuary.territoryRadius(), sentry.world(), target.getLocation().getX(), target.getLocation().getZ());
            var currentPath = mob.getPathfinder().getCurrentPath();
            boolean pathExits = currentPath != null && currentPath.getPoints().stream().anyMatch(point ->
                !TerritoryCalculator.contains(sanctuary.position().orElseThrow(), sanctuary.territoryRadius(), sentry.world(), point.getX(), point.getZ()));
            if (!targetInSanctuary || pathExits || dx*dx+dz*dz > definition.targetRadius()*definition.targetRadius()) service.clearTarget(sentry);
        } else {
    ...
    private void scanTriggers() throws SQLException {
        ...
        for (Sanctuary sanctuary : sanctuaryRepository.findAll()) {
            if (sanctuary.position().isEmpty() || sanctuary.state() != dev.liamtolkkinen.sanctuary.sanctuary.SanctuaryState.ACTIVE) continue;
            var world = Bukkit.getWorld(sanctuary.position().orElseThrow().world()); if (world == null) continue;
            for (Entity entity : world.getEntities()) {
                if (!(entity instanceof LivingEntity living) || service.isManaged(entity)) continue;
                Location l=entity.getLocation();
                if (!TerritoryCalculator.contains(sanctuary.position().orElseThrow(), sanctuary.territoryRadius(), world.getName(), l.getX(), l.getZ())) continue;
                if (entity instanceof Player player) {
                    double dx=l.getX()-(sanctuary.position().orElseThrow().x()+0.5), dz=l.getZ()-(sanctuary.position().orElseThrow().z()+0.5);
                ...
                String key=sanctuary.id()+":"+entity.getUniqueId(); current.add(key);
                if (NEUTRAL_TYPES.contains(entity.getType())) service.trigger(sanctuary,SentryTrigger.NEUTRAL_MOB_ENTERED,living);
                else if (entity instanceof Monster) service.trigger(sanctuary,SentryTrigger.HOSTILE_MOB_ENTERED,living);
            }
        }
        mobPresence.clear(); mobPresence.addAll(current);
    }
}
