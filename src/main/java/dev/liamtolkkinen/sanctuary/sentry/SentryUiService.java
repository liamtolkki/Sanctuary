...
public final class SentryUiService {
    ...
    public void openDashboard(Player player, Sanctuary sanctuary) {
        if (!service.canManage(player, sanctuary)) { player.sendMessage(ChatColor.RED+"Only the Sanctuary owner can manage sentries."); return; }
    ...
    private final class DashboardMenu extends ExtendedInventoryMenu {
        ...
        DashboardMenu(UUID sanctuaryId){super(6,"<gold>Sanctuary Sentries");this.sanctuaryId=sanctuaryId;}
        @Override public void build(ExtendedMenuContext context, ExtendedMenuBuilder menu){
            menu.fillBackground();
            try {
                Sanctuary sanctuary=sanctuaryRepository.findById(sanctuaryId).orElse(null);
                if(sanctuary==null||!service.canManage(context.player(),sanctuary)){menu.set(22,button(Material.BARRIER,"<red>Unavailable",List.of(),null));return;}
                menu.set(4,button(Material.BEACON,"<gold>Global Sentry Behavior",List.of("<gray>These are defaults for sentries with", "<gray>an inherited setting."),click->click.menu().open(new DefaultsMenu(sanctuaryId))));
                ...
                for(int i=0;i<Math.min(slots.length,sentries.size());i++){
                    SentryRecord s=sentries.get(i);SentryDefinition d=service.definition(s).orElse(null);if(d==null)continue;
                    List<String> lore=new ArrayList<>();lore.add("<gray>Status: <white>"+s.state());lore.add("<gray>Range: <white>"+d.targetRadius()+" blocks");lore.add("<gray>Post: <white>"+s.x()+", "+s.y()+", "+s.z());
                    if(s.state()==SentryState.DOWN&&s.respawnAt().isPresent())lore.add("<yellow>Respawn in: "+Math.max(0,Duration.between(Instant.now(),s.respawnAt().orElseThrow()).toSeconds())+"s");
                    menu.set(slots[i],button(s.state()==SentryState.DISABLED?Material.GRAY_DYE:Material.ARMOR_STAND,"<yellow>"+d.displayName(),lore,click->click.menu().open(new SentryMenu(sanctuaryId,s.id()))));
                }
                if(sentries.size()>slots.length)menu.set(49,button(Material.PAPER,"<yellow>More sentries exist",List.of("<gray>Showing first "+slots.length+" in this initial UI."),null));
            } catch(SQLException ex){error(context.player(),ex);}
            menu.set(45,StandardButtons.back(context.theme()));menu.set(53,StandardButtons.close(context.theme()));
        }
    }
    ...
    private final class SentryMenu extends ExtendedInventoryMenu {
        private final UUID sanctuaryId,sentryId;SentryMenu(UUID sid,UUID id){super(6,"<yellow>Sentry");sanctuaryId=sid;sentryId=id;}
        @Override public void build(ExtendedMenuContext context,ExtendedMenuBuilder menu){menu.fillBackground();try{
    ...
    private void error(Player player,Exception ex){player.sendMessage(ChatColor.RED+"Sanctuary sentry UI failed.");logger.log(Level.SEVERE,"Sentry UI failure",ex);}
}
